...
SampleOrderSystem/
  ...
  src/sampleorder/
    ...
    services/
      ...
      calculations.py            # 공식 5종 단일 구현체 (shortage/actual_yield/total_time/pending_demand/stock_status)
    views/
  ...
  tests/
    ...
    test_json_store.py, conftest.py
  CLAUDE.md
  docs/
...
`calculations.py`를 별도 모듈로 분리한 이유: SPEC.md §8이 "5개 공식은 서비스 계층에 단일 구현으로 존재해야 하며 화면/메뉴 간 중복 계산을 두지 않는다"고 명시하므로, `order_service`(승인 시 미리보기 계산)와 `production_service`(큐 계산)가 같은 함수를 import하도록 물리적으로 한 파일에 고정한다.
...
| `repositories/*.py` | `DataPersistence`의 `OrderRepository`/`SampleRepository` | 공통 베이스 클래스 없이 각자 `JsonFileStore`를 감싸 `create/get/list_all/list_by_status/update/delete` 구현, `order_id`는 정적 메서드 `_next_id`로 `ORD-YYYYMMDD-0001` 형식 채번 | 본 프로젝트도 베이스 클래스를 새로 만들지 않고 동일하게 각자 구현(과설계 방지) |
...
| `clock.py` 주입 방식 | `DataMonitor/datamonitor/app.py` | Clock 프로토콜 클래스 없이 `clock_now=datetime.now` 같은 함수 인자 주입 방식 | `production_service`의 진행률 계산 함수도 동일하게 `now_fn: Callable[[], datetime] = datetime.utcnow` 파라미터로 주입 (NFR-4) |
| `monitoring_service.py` | `DataMonitor/datamonitor/monitor.py` | 클래스 `DataMonitor`의 `order_status_counts()`, `pending_demand_by_sample()`, `stock_status(sample, pending_demand=0)`(`STOCK_DEPLETED`/`STOCK_SHORTAGE`/`STOCK_SUFFICIENT` = 고갈/부족/여유), `snapshot()` | 함수 시그니처와 반환 라벨을 그대로 채택, 클래스명은 `MonitoringService`로 변경(패키지 네이밍 통일) |
...
ApprovalController.approve(order_id)
  → OrderService.preview_approval(order_id)
      ...
      3. shortage = calculations.shortage(order.quantity, sample.stock)
      4. if shortage <= 0: return ApprovalPreview(kind=SUFFICIENT)
      5. actual_yield = calculations.actual_yield(shortage, sample.yield_rate)
         total_time = calculations.total_time(sample.avg_production_time, actual_yield)
...
ShippingController.ship(order_id)
  → ShippingService.release(order_id)
      ...
      2. SampleRepository.update(order.sample_id, stock=-order.quantity)
      3. OrderRepository.update(order_id, status=RELEASE)
      4. return ReleaseResult(order, processed_at=now_fn())
```
...
- 저장 파일: `data/samples.json`, `data/orders.json`. 각각 레코드 리스트(JSON 배열)를 통째로 읽고 통째로 쓴다(NFR-2). 트랜잭션/부분 갱신 개념 없음 — 단일 프로세스 콘솔 앱 규모에 적합하다는 PRD.md §3 Out of Scope 전제와 일치.
- 쓰기는 `JsonFileStore.save()` 1곳에서만 발생하며, 모든 Repository의 `update`/`create`/`delete`는 내부적으로 전체 리스트를 메모리에서 수정한 뒤 `save()`를 호출한다. 동시 쓰기 경합은 고려하지 않는다(NFR 범위 밖, 단일 프로세스 가정).
...
| 결정 | 대안 | 선택 이유 |
|---|---|---|
| JSON 파일 저장 | SQLite | PRD.md §3 Out of Scope 및 §5.2 근거 그대로 — 미션1 4개 PoC가 이미 JSON 스키마로 검증됨, 단일 프로세스 규모에 DB 도입은 과설계 |
