...
- `tests_e2e/`: `tests/`와 별도 디렉터리로 분리된 진짜 end-to-end 테스트. `main.py`를 `subprocess`로 실행해 실제 `input()`/`print()` 콘솔 I/O를 그대로 구동하고, `SAMPLEORDER_DATA_DIR` 환경변수로 `tmp_path` 기반 데이터 디렉터리를 격리해 운영 `data/`를 건드리지 않는다. `pyproject.toml`의 `testpaths = ["tests"]`에는 포함하지 않으므로 기본 `pytest` 실행에는 영향을 주지 않고, `pytest tests_e2e`로 별도 실행한다. 메뉴 1~6 전체를 실제 프로세스로 왕복하는 시나리오를 커버한다: 재고 충분/부족 승인 분기(생산라인 큐잉 포함), 거절, 출고 완료, 승인 대기·출고 가능 목록이 비어있는 화면, 시료 등록 시 잘못된 입력에 대한 재입력 루프, 승인 이후 재고가 소진된 경우의 출고 실패(두 번의 CLI 실행 사이에 `samples.json`을 직접 수정해 외부 요인에 의한 재고 변화를 재현).
