...
- `tests_e2e/`: `tests/`와 별도 디렉터리로 분리된 진짜 end-to-end 테스트. `main.py`를 `subprocess`로 실행해 실제 `input()`/`print()` 콘솔 I/O를 그대로 구동하고, `SAMPLEORDER_DATA_DIR` 환경변수로 `tmp_path` 기반 데이터 디렉터리를 격리해 운영 `data/`를 건드리지 않는다. `pyproject.toml`의 `testpaths = ["tests"]`에는 포함하지 않으므로 기본 `pytest` 실행에는 영향을 주지 않고, `pytest tests_e2e`로 별도 실행한다.
