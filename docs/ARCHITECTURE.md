...
SampleOrderSystem/
  ...
  tests/
    ...
    e2e/
      test_cli_lifecycle.py      # main.py를 subprocess로 실행해 실제 stdin/stdout으로 전체 메뉴 흐름 검증
      conftest.py, __init__.py
  CLAUDE.md
  docs/
...
- `tests/e2e/`: `tests/` 하위에 두되 나머지 단위 테스트와는 별도 서브디렉터리로 분리한 진짜 end-to-end 테스트. `main.py`를 `subprocess`로 실행해 실제 `input()`/`print()` 콘솔 I/O를 그대로 구동하고, `SAMPLEORDER_DATA_DIR` 환경변수로 `tmp_path` 기반 데이터 디렉터리를 격리해 운영 `data/`를 건드리지 않는다. `pyproject.toml`의 `testpaths = ["tests"]`에 포함되므로 기본 `pytest` 실행 시 단위 테스트와 함께 자동으로 수집·실행되고, `pytest tests/e2e`로 e2e만 따로 실행할 수도 있다. 메뉴 1~6 전체를 실제 프로세스로 왕복하는 시나리오를 커버한다: 재고 충분/부족 승인 분기(생산라인 큐잉 포함), 거절, 출고 완료, 승인 대기·출고 가능 목록이 비어있는 화면, 시료 등록 시 잘못된 입력에 대한 재입력 루프, 승인 이후 재고가 소진된 경우의 출고 실패(두 번의 CLI 실행 사이에 `samples.json`을 직접 수정해 외부 요인에 의한 재고 변화를 재현).
