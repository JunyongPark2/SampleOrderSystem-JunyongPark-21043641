...
- **FR-3.2 주문 승인**: 목록에서 특정 주문을 선택해 승인한다. 승인 시 해당 시료의 **가용 재고**(현재 재고에서, 이미 `CONFIRMED`/`PRODUCING`으로 승인 확정된 다른 주문의 수량 합을 제외한 값)와 주문 수량을 비교하여 자동으로 분기한다. 아직 승인 여부가 결정되지 않은 `RESERVED` 상태 주문은 가용 재고 계산에서 제외한다(FIFO 원칙 — 먼저 승인된 주문이 그 시점의 가용 재고를 먼저 차지한다).
  - **재고 충분** (`가용 재고 >= quantity`): 즉시 `status`를 `CONFIRMED`로 전환한다. 재고는 이 시점에서 차감하지 않는다 (차감은 출고 시점, FR-6.1 참조).
  - **재고 부족** (`가용 재고 < quantity`): 부족분을 계산해 생산라인 큐에 자동 등록하고 `status`를 `PRODUCING`으로 전환한다.
    - 부족분 = `quantity - 가용 재고`
    - 실생산량 = `ceil(부족분 / yield_rate)`
    - 총생산시간(분) = `avg_production_time * 실생산량`
    - 승인 확인 전, 부족분/실생산량/예상 소요시간을 화면에 안내한 뒤 최종 확인을 받는다 (PDF 예시 화면 참고). 이 확인에서 거절을 선택하면 FR-3.3과 동일하게 즉시 `REJECTED`로 전환된다.
...
- **FR-6.2 출고 실행**: 특정 주문을 선택해 출고를 실행하면:
  1. 차감 전 해당 시료의 `stock`이 주문 수량 이상인지 검증한다. 부족하면 `stock`을 변경하지 않고 오류를 안내한다.
  2. 해당 시료의 `stock`에서 주문 수량만큼 차감한다.
  3. 해당 주문의 `status`를 `CONFIRMED` → `RELEASE`로 전환한다.
  4. 처리 결과(주문번호, 출고수량, 처리일시, 상태 변경)를 표시한다.
