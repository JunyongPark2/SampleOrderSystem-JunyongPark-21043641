...
- [ ] Phase 0~12 파일이 모두 완료 체크됨.
- [ ] `pytest` 전체 스위트가 통과함(NFR-8).
- [ ] PRD.md §9 인수 기준 10개 항목이 모두 수동 재현으로 확인됨.
- [ ] `python main.py`를 재시작해도 이전 세션의 시료/주문 데이터가 유지됨(NFR-1).
- [ ] SPEC.md에 정의된 모든 화면 문구/계산식이 실제 콘솔 출력과 정확히 일치함.
- [ ] 완료 후 본 계획(overview + 13개 Phase 파일)은 `docs/plans/active/`에서 `docs/plans/completed/`(또는 동등한 위치)로 이동하거나 상태를 "완료"로 갱신한다.
